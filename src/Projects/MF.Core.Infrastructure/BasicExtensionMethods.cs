...
namespace MF.Core.Infrastructure
{
    public static class BasicExtensionMethods
    {
        ...
        private static IGESEvent DeserializeEvent(byte[] metadata, byte[] data, string targetTypeName)
        {
            ...
            var continuationId = JObject.Parse(Encoding.UTF8.GetString(metadata)).Property("ContinuationId");
            var metaData = continuationId == null
                               ? new Dictionary<string, object>()
                               : new Dictionary<string, object>() {{continuationId.Name, continuationId.Value}};//, {"ReturnEvent", returnEvent},{"ClientId",clientId}};
