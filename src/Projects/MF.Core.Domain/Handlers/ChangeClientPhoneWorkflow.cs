...
{
    public class ChangeClientPhoneWorkflow : HandlerBase, IHandler
    {
        private readonly IGetEventStoreRepository _getEventStoreRepository;
        public ChangeClientPhoneWorkflow(IMongoRepository mongoRepository,
            ...
            : base(mongoRepository, uiResponsePoster, logger)
        {
            ...
            register(typeof(ChangeClientPhone), correctClientName);
        }

        private void correctClientName(IGESEvent x)
        {
            var _vent = (ChangeClientPhone)x;
